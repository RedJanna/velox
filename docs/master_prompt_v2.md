...
- HTML tasarim dili: Kassandra Oludeniz logo/letterhead hissiyle uyumlu, A4 dikey, premium sade, fume-beyaz-siyaha yakin metinlerle, ince cift cerceveli, dekoratif koseli, logo kilitli, numarali bolum basliklari ve cizgi-dolgulu form alanlari olan belge gorunumu kullanilir; kutulu kart/grid tasarim kullanilmaz. Uc form ayni premium belge dilini paylasir, fakat konaklama "ledger/resmi kayit", restoran "masa/menü davetiyesi", transfer "rota/seyahat belgesi" karakterinde tip bazli farkli template varyantlariyla render edilir.
- Manuel onizleme davranisi: Admin hic bilgi girmese veya bazi alanlar eksik olsa bile HTML onizleme uretilir; eksik bilgiler uydurulmaz, ilgili alanlar bos cizgi olarak gosterilir. Public link uretimi de ayni snapshot mantigiyla calisir; musteriye gonderim icin telefon yoksa gonderim denenmez veya `not_sent` olarak isaretlenir.
...
- Realtime testinde mikrofon sesi OpenAI'a aktarilir; Velox tarafinda yerel ses dosyasi veya gorusme kaydi tutulmaz.
